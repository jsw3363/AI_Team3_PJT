...
class stable_SMOTE:
    ...
    def fit_sample(self, x_dataset, y_dataset):
        ...
        select_column = list(x_dataset.columns[:-1])
        container = pd.DataFrame(columns=x_dataset.columns)

        # Borderline instance selection
        for idx, row in defective.iterrows():
            temp = x_dataset[select_column]
            dist = np.sqrt(((temp - row[select_column]) ** 2).sum(axis=1))
            temp = temp.copy()
        ...
        rround = per_instance / self.z_nearest
        while rround >= 1:
            for idx, row in container.iterrows():
                temp = defective.copy()
                temp["dist"] = np.sqrt(((temp[select_column] - row[select_column]) ** 2).sum(axis=1))
                nn = temp.sort_values("dist").iloc[1:self.z_nearest+1]
                for j in nn.index:
        ...
        if residue > 0:
            residue_samples = container.sample(n=min(residue, len(container)))
            for idx, row in residue_samples.iterrows():
                temp = defective.copy()
                temp["dist"] = np.sqrt(((temp[select_column] - row[select_column]) ** 2).sum(axis=1))
...
# 결과 파일 초기화
os.makedirs("output_data/BORDERLINE_knn", exist_ok=True)

def open_csv(name):
    f = open(f"output_data/BORDERLINE_knn/{neighbor}{name}_borderline_result_on_{classifier}.csv",
             "w", newline="")
    w = csv.writer(f)
